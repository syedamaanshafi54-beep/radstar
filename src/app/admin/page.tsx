      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <Card className="lg:col-span-3">
            ...
            <CardContent className="px-1 sm:px-2">
                <ChartContainer config={chartConfig} className="h-[180px] sm:h-[300px] w-full overflow-x-hidden">
                <ComposedChart data={monthlyChartData} margin={{ left: 0, right: 0, top: 10, bottom: 0 }}>
                    <defs>
                        ...
                        <stop offset="95%" stopColor="var(--color-orders)" stopOpacity={0.1}/>
                         ...
                         <filter id="shadowSales" x="-20%" y="-20%" width="140%" height="140%">
                            <feDropShadow dx="0" dy="2" stdDeviation="2" floodColor="var(--color-sales)" floodOpacity="0.3"/>
                        </filter>
                    </defs>
                    ...
                    />
                    <Legend />
                    <Bar dataKey="orders" yAxisId="orders" fill="url(#colorOrders)" radius={4} barSize={20} />
                    <Line type="monotone" yAxisId="sales" dataKey="sales" stroke="var(--color-sales)" strokeWidth={2} dot={false} filter="url(#shadowSales)"/>
                </ComposedChart>
                </ChartContainer>
            </CardContent>
        </Card>
        ...
        <div className="relative overflow-hidden">
          <AnimatePresence initial={false} custom={swipeDirection}>
            <motion.div
              ...
              initial={{ x: swipeDirection === 1 ? 40 : -40, opacity: 0 }}
              animate={{ x: 0, opacity: 1 }}
              exit={{ x: swipeDirection === 1 ? -40 : 40, opacity: 0 }}
              ...
              onDragEnd={(_, info) => {
                ...
                const currentIndex = mobileTabs.findIndex((t) => t.key === activeMobileTab);
                if (offsetX < -threshold && currentIndex < mobileTabs.length - 1) {
                  handleMobileTabChange(mobileTabs[currentIndex + 1].key);
