...
import { ArrowRight, Eye, ShoppingCart, Minus, Plus, Sparkles, Loader2, Info } from 'lucide-react';
...
import { WithId, useCollection, useDoc, useFirestore, useMemoFirebase } from '@/firebase';
  ...
  return (
    <div className="flex flex-col bg-background">
            ...
            {heroSlides.map((_, index) => (
              <button
                ...
                className={`w-2 h-2 md:w-3 md:h-3 rounded-full transition-colors ${
                  index === current ? 'bg-primary' : 'bg-white/50 hover:bg-white/75'
                }`}
              />
            ))}
          </div>
      ...
      <div id="main-content" className="bg-background relative z-20">
        <section id="products" className="py-12 md:py-16 pt-8">
            <motion.div
              initial="initial"
              whileInView="animate"
      ...
      <Card className="overflow-hidden group flex flex-col h-full rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300">
        <CardContent className="p-0 flex-1 flex flex-col">
          ...
          <div className="p-4 flex flex-col flex-1">
            <DialogTrigger asChild>
                <h3 className="font-headline text-2xl md:text-3xl font-bold flex-1 cursor-pointer hover:text-primary transition-colors">
                    {product.name}
                </h3>
            </DialogTrigger>
