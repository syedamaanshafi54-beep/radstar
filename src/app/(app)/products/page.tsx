...
function ProductsGrid() {
  ...
  const productsQuery = useMemoFirebase(() => {
    if (filter) {
      return query(productsCollection, where('category', '==', filter));
    }
    return productsCollection;
  }, [productsCollection, filter]);

  const { data: firestoreProducts, isLoading: productsLoading } = useCollection<Product>(productsQuery);
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    if (quantity === 0) {
      ...
      addToCart(product, 1, selectedVariant);
       toast({
        title: "Added to cart",
        description: `1 x ${product.name} ${selectedVariant ? `(${selectedVariant.name})` : ''} has been added.`,
        duration: 5000,
    ...
    toast({
      title: "Added to cart",
      description: `${quantity} x ${product.name} ${selectedVariant ? `(${selectedVariant.name})` : ''} have been added.`,
      duration: 5000,
      action: <ToastAction altText="View Cart" onClick={() => router.push('/cart')}>View Cart</ToastAction>,
    });
  };
      ...
      <Card className="overflow-hidden group flex flex-col h-full rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300">
        <CardContent className="p-0 flex-1 flex flex-col">
          ...
          <div className="p-6 flex flex-col flex-1">
            <DialogTrigger asChild>
                <h3 className="font-headline text-2xl flex-1 cursor-pointer">
                  {product.name}
                </h3>
            </DialogTrigger>
